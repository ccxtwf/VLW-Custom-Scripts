...
const __rxPwtTable =
  /(?<head>{\|\s*class=["']sortable\s+producer-table["']\s*\n\|-[^{}\n]*?\n!\s*\{\{\s*[Pp]wt[ _]head\s*\}\}\s*\n)(.*?(?!\|\}\}))\|\}/gs;
const __rxAwtTable =
...
function parseDplOutputToList(output: string): string[] {
  ...
  const res = (
    (dom.window.document.querySelector("div.mw-parser-output > p") || {}).innerHTML || ""
  ).trim();
  if (res === "") return [];
...
function getSortedValue(pwtTemplateInput: string, forAlbum: boolean = false): string {
  ...
  let manualKanji = manualKanjiMatch === null ? "" : manualKanjiMatch[1]; // oxlint-disable-line no-unused-vars
...
export interface IProducerPageBotMixin {
  ...
  filterPagesThatHaveBeenTranscludedUsingRedirects: (
    pages: string[],
    prodPageTitle: string,
    forAlbums?: boolean,
...
export const producerPageBotMixin: IProducerPageBotMixin = {
  ...
  async filterPagesThatHaveBeenTranscludedUsingRedirects(
    this: IProducerPageBotMixin,
    pages: string[],
    prodPageTitle: string,
    forAlbums: boolean = false,
  ) {
    if (pages.length === 0) {
    ...
    for await (let json of q) {
      const untranscludedPages = json
        .query!.pages.filter((page: ApiPage) => {
  ...
  async getSongPagesNotOnPage(this: IProducerPageBotMixin, prodCat: string, prodPageTitle: string) {
    ...
    const pages = await this.filterPagesThatHaveBeenTranscludedUsingRedirects(
      parseDplOutputToList(res),
      prodPageTitle,
    );
    return pages as string[];
  },
  async getAlbumPagesNotOnPage(
    this: IProducerPageBotMixin,
    prodCat: string,
    prodPageTitle: string,
  ) {
  ...
  updatePwt(this: IProducerPageBotMixin, pageContents: string, missingSongs: string[]) {
    ...
    const extractMatchProperties = (m: RegExpExecArray) => {
      return {
        fullmatch: m[0],
        input: m[1],
    ...
    const pwtSongs = Array.from(pwtTableWikitext.matchAll(__rxPwtRowTemplate)).map(
      extractMatchProperties,
    );
    ...
    let newPwtTableWikitext =
      pwtTables[0]!.groups!["head"] + pwtSongs.map((m) => m.fullmatch).join("\n") + "\n|}";
  ...
  updateAwt(this: IProducerPageBotMixin, pageContents: string, missingAlbums: [string, boolean][]) {
    ...
    switch (awtTables.length) {
      ...
      case 1:
        ...
        awtAlbums = Array.from(awtTableWikitext.matchAll(__rxAwtRowTemplate)).map(
          extractMatchProperties,
        );
        for (let [missingAlbum, _] of missingAlbums) {
          ...
          awtAlbums.splice(addToIndex, 0, {
            fullmatch: awtTemplate,
            input: missingAlbum,
            sortValue,
          });
        }
        newAwtTableWikitext =
          awtTables![0].groups!["head"] + awtAlbums.map((m) => m.fullmatch).join("\n") + "\n|}";
      ...
      default:
        ...
        awtCompilationAlbums = Array.from(
          awtTableWikitextCompilations.matchAll(__rxAwtRowTemplate),
        ).map(extractMatchProperties);
        for (let [missingAlbum, isCompilation] of missingAlbums) {
          ...
          searchInTable.splice(addToIndex, 0, {
            fullmatch: awtTemplate,
            input: missingAlbum,
            sortValue,
          });
        ...
        newAwtTableWikitext =
          awtTables![0].groups!["head"] + awtAlbums.map((m) => m.fullmatch).join("\n") + "\n|}";
        newAwtTableWikitextCompilations =
          awtTables![1].groups!["head"] +
          awtCompilationAlbums.map((m) => m.fullmatch).join("\n") +
          "\n|}";
        pageContents = pageContents.replace(awtTableWikitext, newAwtTableWikitext);
        pageContents = pageContents.replace(
          awtTableWikitextCompilations,
          newAwtTableWikitextCompilations,
        );
        return pageContents;
    }
