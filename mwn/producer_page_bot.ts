...
const producersWithDiscographySubpages = JSON.parse(
  fs.readFileSync("./producers_with_split_discographies.json", { encoding: "utf-8", flag: "r" }),
);
...
class ProducerPageBot extends Mwn {
  ...
  constructor(
    options: MwnOptions,
    config: IProducerPageConfig,
    cliOptions: IProducerPageBotCliOptions,
  ) {
    super(options);
  ...
  getProducerCategory = (producerPageBotMixin as IProducerPageBotMixin).getProducerCategory.bind(
    this,
  );
  filterPagesThatHaveBeenTranscludedUsingRedirects = (
    producerPageBotMixin as IProducerPageBotMixin
  ).filterPagesThatHaveBeenTranscludedUsingRedirects.bind(this);
  getSongPagesNotOnPage = (
    producerPageBotMixin as IProducerPageBotMixin
  ).getSongPagesNotOnPage.bind(this);
  getAlbumPagesNotOnPage = (
    producerPageBotMixin as IProducerPageBotMixin
  ).getAlbumPagesNotOnPage.bind(this);
  ...
  async treatPage(this: ProducerPageBot, { title, revisions }: ApiPage): Promise<void> {
    try {
      ...
      if (songPagesNotInCategory.length > 0) {
        Mwn.log(
          `[I] ${title}: Found ${songPagesNotInCategory.length} song page(s): ${songPagesNotInCategory.join(", ")}`,
        );
      ...
      if (albumPagesNotInCategory.length > 0) {
        Mwn.log(
          `[I] ${title}: Found ${albumPagesNotInCategory.length} album page(s): ${albumPagesNotInCategory.map(([album, isCompilation]: [string, boolean]) => `${album}${isCompilation ? " [COMPILATION]" : ""}`).join(", ")}`,
        );
  ...
  async run(this: ProducerPageBot): Promise<void> {
    if (this.userOptions["dry-run"]) {
      Mwn.log(
        `[I] Running the producer page bot as a simulation. No live changes will be made on the wiki.`,
      );
    }
...
async function initBot() {
  ...
  const bot = new ProducerPageBot(
    {
      ...wikiProfile.miscConfig as Record<any, any>,
    ...
    },
    {
      ...
      producerPagesWithDiscogSubpages:
        producersWithDiscographySubpages as IProducerPagesWithDiscogSubpages,
    },
    cliOptions,
  );
