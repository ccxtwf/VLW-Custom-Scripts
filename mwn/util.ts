...
export function setLoggingConfig({
  prefix = "mwn-log",
  dualStream = false,
}: { prefix?: string; dualStream?: boolean } = {}) {
  //@ts-ignore
  const __dirname = import.meta.dirname;
  ...
  const _fs = createWriteStream(resolve(logsFolderPath, `${prefix}-${currentDate}.log`), {
    flags: "a",
    encoding: "utf8",
  });
  _ps.pipe(_fs);
...
export async function integratedLogin(bot: Mwn) {
  try {
    if (!bot.options.OAuth2AccessToken) {
      ...
      Mwn.log(
        styleText("yellow", `[I] Logging into ${bot.options.apiUrl} as ${bot.options.username}`),
      );
      await bot.login({
      ...
      Mwn.log(
        styleText(
          "yellow",
          `[I] Authenticating OAuth credentials by checking in with ${bot.options.apiUrl}`,
        ),
      );
      bot.initOAuth();
